...
\fancyhead[RO,LE]{Herhaling Reële Functies}
\fancyhead[RE,LO]{}
...
\fill [color=black] (-1.5,0) circle (2.5pt);
\end{scriptsize}
\end{tikzpicture}
\end{center}
...
\begin{oefening}
Lees van volgende grafiek het domein, het bereik, de nulwaarde(n), het tekenverloop, het stijgen en dalen en de extrema.
...
\draw[line width=1.2pt, smooth,samples=100,domain=-5.0:5.0] plot(\x,{15/((\x)^2+3)});
...
\draw[line width=1.2pt, smooth,samples=100,domain=-5.0:5.0] plot(\x,{(\x)^3+3*(\x)^2+2});
