...
Dus door de afgelegde afstand te delen door de lengte van het tijdsinterval vinden we de gemiddelde snelheid. Eigenlijk berekenen we zo de gemiddelde verandering over dat interval. Dit wordt de differentiequotiënt in dat interval genoemd.

\subsection{Definitie}

\paragraph{Differentiequotiënt}
\begin{mdframed}
De differentiequotiënt $\dfrac{\Delta y}{\Delta x}$ van een functie $f$ voor een interval $[a, b]$ is de gemiddelde verandering van deze functie in interval. We bereken
...
De differentiequotiënt geeft de richtingscoëfficiënt van het lijnstuk dat door de punten $(a,f(a))$ en $(b,f(b))$ loopt, zie op bovenstaande figuur.
...
Beschouw de eerstegraadsfunctie $f(x)=2x+1$
...
\draw (0.26,0.9) node[anchor=north west] {$\Delta x$};
\draw (1.24,2.18) node[anchor=north west] {$\Delta y$};
\end{tikzpicture}
\end{center}
